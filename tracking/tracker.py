...
def handle_tracked_objects(config, delta, height, total, total_down, total_up, tracked_objects, coords_left, coords_right):
    ...
    for (object_id, data) in tracked_objects.items():
        ...
        if data.get('initialPositionUp') is None:
            ...
            if direction < 0 and coords_right > centroid[0] > coords_left and centroid[1] < height // 2 and not data['initialPositionUp']:
                total_down += 1
                delta += 1
            ...
            elif direction < 0 and (centroid[0] < coords_left or centroid[0] > coords_right) and centroid[1] < height // 2 and not data['initialPositionUp']:
                data['initialPositionUp'] = not data['initialPositionUp']

            elif direction > 0 and coords_right > centroid[0] > coords_left and centroid[1] > height // 2 and data['initialPositionUp']:
                total_up += 1
                delta -= 1
                log_event(config, f"EXIT {data['type']} {object_id}", total_up, delta, direction, height,
                          centroid[1], data['initialPositionUp'])
                data['initialPositionUp'] = not data['initialPositionUp']
            elif direction > 0 and (centroid[0] < coords_left or centroid[0] > coords_right) and centroid[1] > height // 2 and data['initialPositionUp']:
                data['initialPositionUp'] = not data['initialPositionUp']
